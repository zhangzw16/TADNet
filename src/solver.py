...
class Solver(object):
    ...
    def __init__(self, data, model, optimizer, args):
        ...
        self.tr_loader2 = data['tr_loader2']
        ...
        # logging
        self.multi_data = args.multi_data
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            start = time.time()
            if epoch % 2 != 0 or self.multi_data == 0:  
                tr_avg_loss = self._run_one_epoch(epoch)
            else:
                tr_avg_loss = self._run_one_epoch(epoch, multi_data=True)
    ...
    def _run_one_epoch(self, epoch, cross_valid=False, multi_data=False):
        start = time.time()
        total_loss = 0
        if multi_data:
            data_loader = self.tr_loader2 if not cross_valid else self.cv_loader
        else:
        ...
        for i, (data) in enumerate(data_loader):
            ...
            mask_len = 0
            # add mask
            for j in range(padded_mixture.shape[0]):
                masked_array, pos = mask_array(mask_len, padded_mixture[j])
                position += [pos]
                padded_mixture[j] = masked_array
